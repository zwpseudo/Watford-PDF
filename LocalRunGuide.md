...
### Step 4: Clone and Build Watford-PDF

```bash
cd ~/.git &&\
git clone https://github.com/zwpseudo/Watford-PDF.git &&\
cd Watford-PDF &&\
chmod +x ./gradlew &&\
./gradlew build
```
...
You can move this file to a desired location, for example, `/opt/Watford-PDF/`.
You must also move the Script folder within the Watford-PDF repo that you have downloaded to this directory.
This folder is required for the python scripts using OpenCV

```bash
sudo mkdir /opt/Watford-PDF &&\
sudo mv ./build/libs/Watford-PDF-*.jar /opt/Watford-PDF/ &&\
sudo mv scripts /opt/Watford-PDF/ &&\
...
### Step 7: Run Watford-PDF

```bash
./gradlew bootRun
or
java -jar /opt/Watford-PDF/Watford-PDF-*.jar
```
...
image=$(pwd)/docs/watford-transparent.svg

cat > ~/.local/share/applications/Watford-PDF.desktop <<EOF
[Desktop Entry]
Name=Watford PDF;
GenericName=Launch WatfordPDF and open its WebGUI;
Category=Office;
...
### Optional: Run Watford-PDF as a service

First create a .env file, where you can store environment variables:
```
touch /opt/Watford-PDF/.env
```
...
```
nano /etc/systemd/system/watfordpdf.service
```
...
```
[Unit]
Description=Watford-PDF service
...
EnvironmentFile=/opt/Watford-PDF/.env
WorkingDirectory=/opt/Watford-PDF
ExecStart=/usr/bin/java -jar Watford-PDF-0.17.2.jar
...
```
sudo systemctl enable watfordpdf.service
```

See the status of the service:
```
sudo systemctl status watfordpdf.service
```

Manually start/stop/restart the service:
```
sudo systemctl start watfordpdf.service
sudo systemctl stop watfordpdf.service
sudo systemctl restart watfordpdf.service
```
...
```bash
export APP_HOME_NAME="Watford PDF"
or
-DAPP_HOME_NAME="Watford PDF" 
```
